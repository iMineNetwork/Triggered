...
public class PlayerBattleLoseCondition extends PlayerBattleEndCondition {

    @JsonCreator
    public PlayerBattleLoseCondition(@JsonProperty("trainerTag") String trainerTag, @JsonProperty("requirements") List<Requirement> requirements) {
    ...
    public boolean matchesCondition(Player player, Set<BattleParticipant> battleParticipantSet, BattleResults battleResults) {
        return (battleResults.equals(BattleResults.DEFEAT) || battleResults.equals(BattleResults.FLEE) || battleResults.equals(BattleResults.DRAW)) && battleParticipantSet.stream().anyMatch(battleParticipant -> {
